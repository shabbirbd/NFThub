...
const Wallets = () => {
    ...
    return (
        <div className='container mx-auto py-10 grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8 px-10'>
            {
                wallets?.map((item, i)=>(
                    <div className='group rounded-xl bg-white shadow hover:shadow-md p-6 text-center mt-10 transition-all duration-500' key={i}>
                        <div className='relative -mt-16'>
                            <img src={item.icon} alt="icon" className='h-20 w-20 rounded-full shadow-md mx-auto p-3' />
                            <h5 className="text-lg font-semibold mt-5">{item.name}</h5>
